// Main JavaScript functionality (assets copy)
document.addEventListener('DOMContentLoaded', function(){
  // المتغيرات العامة
  const modals = {};
  let activeModal = null;
  // currently displayed country id inside modal
  ...
  document.addEventListener('keydown', function(e){ if(e.key === 'Escape' && activeModal){ hideModal(activeModal); }});

  var navLinks = document.querySelectorAll('.nav-links a');
  navLinks.forEach(function(a){ a.addEventListener('click', function(){ if(menu && window.getComputedStyle(document.querySelector('.nav-toggle')).display !== 'none'){ menu.style.display = ''; if(toggle) toggle.setAttribute('aria-expanded', 'false'); } }); });

  var cta = document.querySelector('.btn.primary');
  if(cta){ cta.addEventListener('click', function(e){ e.preventDefault(); var target = document.querySelector(this.getAttribute('href')); if(target){ target.scrollIntoView({behavior:'smooth'}); } }); }

  var watch = document.getElementById('watchIntro');
  if(watch){
    var modal = document.getElementById('introModal');
    var closeBtn = modal && modal.querySelector('.modal-close');
    watch.addEventListener('click', function(){ if(modal){ modal.hidden = false; modal.querySelector('.modal-panel').focus(); } else { alert('عرض المقدمة قيد الإعداد. سيتم فتح فيديو هنا لاحقًا.'); } });
    if(closeBtn){ closeBtn.addEventListener('click', function(){ modal.hidden = true; watch.focus(); }); }
    document.addEventListener('keydown', function(e){ if(e.key === 'Escape' && modal && !modal.hidden){ modal.hidden = true; watch.focus(); } });
  }
  ...
  try{
    ...
    if(!user){
      ...
      if(logoutBtnHeader){ logoutBtnHeader.textContent = 'تسجيل الدخول'; logoutBtnHeader.addEventListener('click', function(){ window.location.href = 'login.html'; }); }
    } else {
      ...
      if(avatarBadges){
        ...
        // do not display the beginner badge on the homepage
      }
      if(logoutBtnHeader){ logoutBtnHeader.textContent = 'تسجيل الخروج'; logoutBtnHeader.addEventListener('click', function(){ localStorage.removeItem('userData'); window.location.href = 'login.html'; }); }
    }

    const avatarBtn = document.getElementById('avatar-btn');
    if(avatarBtn && avatarMenu){
      avatarBtn.addEventListener('click', function(e){ e.stopPropagation(); const open = avatarMenu.hasAttribute('hidden') ? false : true; if(open){ avatarMenu.hidden = true; avatarBtn.setAttribute('aria-expanded','false'); } else { avatarMenu.hidden = false; avatarBtn.setAttribute('aria-expanded','true'); } });
  ...
  window.addEventListener('profileUpdated', function(e){
    try{
      ...
      if(avatarBadges){ avatarBadges.innerHTML = ''; const quizScore = parseInt(localStorage.getItem('quizScore')||'0',10); if(quizScore>=80){ const span=document.createElement('span'); span.textContent='🏅 وسام القائد الذهبي'; avatarBadges.appendChild(span);} else if(quizScore>=50){ const span=document.createElement('span'); span.textContent='🎖️ وسام الباحث الفضي'; avatarBadges.appendChild(span);} }
  ...
  window.addEventListener('langChanged', function(e){
    try{
      ...
      if(activeModal && currentModalCountryId){ showModal(activeModal, currentModalCountryId); }
    }catch(err){ console.warn('langChanged handler error', err); }
  });
